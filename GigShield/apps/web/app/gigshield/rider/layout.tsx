...
export default function RiderLayout({ children }: { children: React.ReactNode }) {
    ...
    const handleLogout = async () => {
        await logout();
        router.push('/gigshield/rider/login');
    };
      ...
      `}</style>
            ...
            <nav className="bg-stone-950/40 backdrop-blur-2xl docked full-width top-0 sticky z-50 flex justify-between items-center w-full px-6 h-16 no-border shadow-[0_8px_32px_rgba(78,0,13,0.15)]">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-epilogue font-black text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-pink-600 text-2xl">GigShield</span>
                </div>
                <div className="hidden md:flex items-center gap-8">
                    <Link className={`${pathname.includes('homepage') || pathname === '/gigshield/rider' ? 'text-rose-400 font-bold' : 'text-stone-400'} font-epilogue text-lg tracking-tight hover:bg-rose-500/10 transition-colors px-3 py-1 rounded-lg`} href={`/gigshield/rider/homepage${userId ? `?user=${userId}` : ''}`}>Home</Link>
                    <Link className={`${pathname.includes('shield') ? 'text-rose-400 font-bold' : 'text-stone-400'} font-epilogue text-lg tracking-tight hover:bg-rose-500/10 transition-colors px-3 py-1 rounded-lg`} href={`/gigshield/rider/shield${userId ? `?user=${userId}` : ''}`}>Shield</Link>
                    <Link className={`${pathname.includes('earnings') ? 'text-rose-400 font-bold' : 'text-stone-400'} font-epilogue text-lg tracking-tight hover:bg-rose-500/10 transition-colors px-3 py-1 rounded-lg`} href={`/gigshield/rider/earnings${userId ? `?user=${userId}` : ''}`}>Earnings</Link>
                </div>
                {isAuthenticated ? (
            ...
            <footer className="bg-stone-950 w-full py-12 px-6 mt-auto border-t border-stone-800/50 hidden md:block">
                <div className="flex flex-col md:flex-row justify-between items-center gap-6 max-w-7xl mx-auto">
                    <div className="flex flex-col items-center md:items-start gap-4">
                        <span className="font-epilogue font-bold text-rose-500 text-2xl tracking-tight">GigShield</span>
                        <p className="font-manrope text-sm text-stone-400">© 2026 GigShield. Officially partnered with Zomato.</p>
                    </div>
            ...
            <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-6 pt-3 bg-stone-950/60 backdrop-blur-3xl rounded-t-[32px] shadow-[0_-8px_24px_rgba(78,0,13,0.2)]">
                <Link href={`/gigshield/rider/homepage${userId ? `?user=${userId}` : ''}`} className="flex flex-col items-center justify-center relative">
                    <div className={`${pathname.includes('homepage') || pathname === '/gigshield/rider' ? 'bg-gradient-to-br from-rose-500 to-pink-700 text-white rounded-2xl px-5 py-2 scale-110 -translate-y-1 shadow-lg shadow-rose-900/40' : 'text-stone-500 py-2'} flex flex-col items-center transition-all duration-300`}>
                        <span className="material-symbols-outlined" data-icon="home">home</span>
                        <span className="font-manrope text-[11px] font-semibold uppercase tracking-wider mt-1">Home</span>
                    </div>
                </Link>
                <Link href={`/gigshield/rider/shield${userId ? `?user=${userId}` : ''}`} className="flex flex-col items-center justify-center relative">
                ...
                <Link href={`/gigshield/rider/earnings${userId ? `?user=${userId}` : ''}`} className="flex flex-col items-center justify-center relative">
                ...
                <Link href={`/gigshield/rider/account${userId ? `?user=${userId}` : ''}`} className="flex flex-col items-center justify-center relative">
